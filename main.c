  ...
  switch (ChoixCategorie1){
    ...
    case 2: /* Solo  */
      ...
      if((NombreHaut+NombreLong)>=12)
         GrandeTaille=0;
      if(!(4+NombreHaut*8 >= LINES||9+NombreLong*9 >=COLS)){
        CreationMemory(NombreHaut,NombreLong,NombreElement,NombreGroupe,Contenuboite,GrandeTaille);
        LANCEMENT_JEU(NombreLong,NombreHaut,NombreElement,NombreGroupe,Contenuboite,GrandeTaille,0,mode);
      }
      else{
        clear();
        mvprintw(LINES/2,COLS/2-22,"Ecran trop petit essayer une autre diposition");
        refresh();
        sleep(5);
      }
      break; 
    ...
    case 5:/* IA */
      clear();
      EtatPc=5;
      mvprintw(LINES/2,COLS/2-8,"Arrive plus tard");
      clear();
      sleep(4);
      break;
    case 6:
      clear();
      ChangementMode(menu("Mode","Chiffre","Alphabet","Dessin","Exam",(char*)0)-1);
