...
void AfficheLettres(char *cara,int haut,int lon){
  ...
  int affichage;
  FILE* fichier = NULL;
    if((cara[0]>='a' && cara[0]<'z')||(cara[0]>='A' && cara[0]<'Z')||(cara[0]>='1' && cara[0]<='9')){

      if(cara[0]>='a' && cara[0]<'z') {
        sprintf(final,"alphabet_p/%c.txt",cara[0]);/*Permet d'avoir le chemin et le nom du fichier pour ouvrir la lettre*/
      }
      else if(cara[0]>='1' && cara[0]<='9'){
        sprintf(final,"chiffre/%c.txt",cara[0]);
      }
      else{
       sprintf(final,"alphabet_g/%c.txt",cara[0]);
     }
        fichier = fopen( final, "r"); /*Ouvre le fichier*/
        ...
        if (fichier==NULL)
          ...
          for(i=0;i<5;i++){
            for(j=0;j<5;j++){
              ...
              if (affichage==3)
               mvprintw(i+decalon,j+decahaut,"█");/*Affiche le caractère*/
    ...
    else if (cara[0]=='z'){ /*Effacement de la case */
           for(i=0;i<5;i++){
            for(j=0;j<5;j++){
...
void ContenuBoiteAffiche(int ContenuCoord,int mode,char Text[6]){
    ...
    switch (mode){
      case 0:
        sprintf(Text, "%c",ContenuCoord+48 );
        break;
      case 1:
        sprintf(Text, "%c",ContenuCoord+96);
        break;
      case 2:
        sprintf(Text, "%c",ContenuCoord+64);
        break;
      case 3:
...
void LANCEMENT_JEU(int LongueurVoulu,int HauteurVoulu,int NombreElement,int NombreGroupe,int Contenuboite[16][4],int GrandeTaille ,int difficulte,int mode)
{
 ...
 /* Initialisation des scores */
        ...
        while( i<NombreElement){
            if(JoueurEnCours==EtatPc || EtatPc==2 || EtatPc==3||(EtatPc==5&&JoueurEnCours==0)){
              ...
              do{
                ...
                refresh();
                /* Affiche la lettre */
                if (GrandeTaille==0) mvprintw(5+HauteurTableau*3,9+LongueurTableau*5,"%s",ContenuAffiche);
  ...
  if(EtatPc==2){
    mvprintw(4+HauteurVoulu*8,9+LongueurVoulu*9,"Partie terminee appuyez sur q pour continuer, vous avez mis %d secondes à faire le memory",tempsD); refresh();    
  }
  else{
